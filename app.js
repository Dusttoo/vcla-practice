...
class VCLAStudyApp {
  ...
  showWelcomeScreen() {
    ...
    container.innerHTML = `
            <div style="background-color: #fff; color: #000;" class="welcome-screen fade-in">
                ...
                <div class="welcome-content">
                    ...
                    <div class="features">
                        <div class="feature">
                            <h3 style="color: #000;">🎯 Adaptive Learning</h3>
                            <p>Questions adapt to your skill level</p>
                        </div>
                        <div class="feature">
                            <h3 style="color: #000;">📊 Progress Tracking</h3>
                            <p>Monitor your improvement over time</p>
                        </div>
                        <div class="feature">
                            <h3 style="color: #000;">🧠 Detailed Explanations</h3>
                            <p>Learn from mistakes with step-by-step guidance</p>
                        </div>
                        <div class="feature">
                            <h3 style="color: #000;">🔀 Customizable Practice</h3>
                    ...
                    <div class="quick-stats">
                        ...
                        <div class="quick-stat">
                            <span class="stat-number">${
                              new Set(questionBank.map((q) => q.topic)).size
                            }</span>
  ...
  startSession() {
    ...
    if (selectedCount === "all") {
      ...
      this.currentQuestions = shuffledQuestions.slice(
        0,
  ...
  showSessionInfo() {
    const difficulty =
      this.currentDifficulty === "all"
        ? "All Levels"
        : this.formatDifficulty(this.currentDifficulty);
    const topic =
      this.currentTopic === "all"
        ? "All Topics"
        : this.formatTopicName(this.currentTopic);
    ...
    container.innerHTML = `
            <div class="slide-up">
                <div class="question-header">
                    ${this.getTopicIcon(question.topic)} ${this.formatTopicName(
      question.topic
    ).toUpperCase()} - Question ${this.currentQuestionIndex + 1}
  ...
  updateStats() {
    document.getElementById(
      "score"
    ).textContent = `${this.score}/${this.totalAnswered}`;
  ...
  updateProgress() {
    if (this.currentQuestions.length > 0) {
      ...
      document.getElementById("progressText").textContent = `${Math.round(
        progress
      )}% Complete (${this.currentQuestionIndex + 1}/${
  ...
  showFinalReview() {
    ...
    reviewContainer.innerHTML = `
            ...
            <div class="review-sections">
                ${
                  this.missedQuestions.length > 0
                    ? this.buildMissedQuestionsReview()
                    : ""
                }
    ...
    this.missedQuestions.forEach((missed, index) => {
      ...
      html += `
                <div class="review-question-card missed">
                    ...
                    <div class="question-topic">${this.getTopicIcon(
                      question.topic
                    )} ${this.formatTopicName(
        question.topic
      )} - ${question.difficulty.toUpperCase()}</div>
                    <div class="question-text">${question.question}</div>
                    ${
                      question.originalSentence
                        ? `<div class="original-sentence">${question.originalSentence}</div>`
                        : ""
                    }
                    
                    <div class="answer-comparison">
                        <div class="your-answer incorrect">
                            <strong>Your Answer:</strong> ${String.fromCharCode(
                              65 + missed.userAnswer
                            )}) ${question.choices[missed.userAnswer]}
                        </div>
                        <div class="correct-answer correct">
                            <strong>Correct Answer:</strong> ${String.fromCharCode(
                              65 + missed.correctAnswer
                            )}) ${question.choices[missed.correctAnswer]}
                        </div>
                    </div>
        ...
        question.topic
                    ...
                    <div class="answer-summary ${
                      isCorrect ? "correct" : "incorrect"
                        ...
                        ${
                          isCorrect
                            ? `<div class="status-correct">✅ Correct! You chose: ${String.fromCharCode(
                                65 + userAnswerIndex
                              )}) ${question.choices[userAnswerIndex]}</div>`
                            : `<div class="status-incorrect">❌ Your answer: ${String.fromCharCode(
                                65 + userAnswerIndex
                              )}) ${question.choices[userAnswerIndex]}<br>
                             ✅ Correct answer: ${String.fromCharCode(
                               65 + question.correct
                             )}) ${question.choices[question.correct]}</div>`
                        }
                    </div>
  ...
  findUserAnswer(question, questionIndex) {
    ...
    const missed = this.missedQuestions.find(
      (m) => m.question.id === question.id
    );
    if (missed) {
    ...
    this.missedQuestions.forEach((missed, index) => {
      ...
      reviewHtml += `
                <div class="review-question">
                    <h4>Question ${index + 1}: ${this.formatTopicName(
        question.topic
      )}</h4>
                    <div class="review-question-text">${question.question}</div>
                    <div class="review-choices">
                        <div class="review-choice incorrect">Your answer: ${String.fromCharCode(
                          65 + missed.userAnswer
                        )}) ${question.choices[missed.userAnswer]}</div>
                        <div class="review-choice correct">Correct answer: ${String.fromCharCode(
                          65 + missed.correctAnswer
                        )}) ${question.choices[missed.correctAnswer]}</div>
                    </div>
                    ${this.buildExplanation(question)}
                </div>
            `;
  ...
  exportResults() {
    ...
    const exportFileDefaultName = `vcla-study-results-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
  ...
  shuffleQuestions() {
    if (this.sessionActive && this.currentQuestions.length > 0) {
      ...
      const remaining = this.currentQuestions.slice(
...
function updateWelcomeScreen(availableCount, selectedCount) {
  ...
  if (container.querySelector(".welcome-screen")) {
    const countDisplay =
      selectedCount === "all"
        ? availableCount
        : Math.min(parseInt(selectedCount), availableCount);
